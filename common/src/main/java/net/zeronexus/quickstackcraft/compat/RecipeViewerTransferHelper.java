...
public final class RecipeViewerTransferHelper {
    ...
    private static List<StackGroup> collectAvailableItems(Player player, long externalLockedMask) {
        ...
        for (int slot = InventoryUtil.HOTBAR_START; slot < InventoryUtil.MAIN_INV_END; slot++) {
            if (isProtectedInventorySlot(slot, externalLockedMask)) {
                continue;
            }
            addStack(available, inventory.getItem(slot));
        }

        if (player.containerMenu instanceof CraftingMenu craftingMenu) {
            for (int slot = 1; slot <= 9; slot++) {
                addStack(available, craftingMenu.getSlot(slot).getItem());
            }
        } else {
            for (int slot = 1; slot <= 4; slot++) {
                addStack(available, player.inventoryMenu.getSlot(slot).getItem());
            }
        }

        NearbyItemsCache.forEachItem((item, count) ->
                addStack(available, new ItemStack(item, count)));
        return available;
    }
    ...
    private static void addStack(List<StackGroup> available, ItemStack stack) {
        ...
        for (int i = 0; i < available.size(); i++) {
            ...
            if (ItemStack.isSameItemSameComponents(group.stack(), stack)) {
                available.set(i, new StackGroup(
                        group.stack(), saturatedAdd(group.count(), stack.getCount())));
                return;
            }
        }
